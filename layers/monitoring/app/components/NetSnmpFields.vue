...
import type { SnmpFormFields } from '~~/layers/monitoring/app/utils/netSnmp'
...
//
// `keepBlank`: set when editing an existing device/host - the server never
// returns stored credentials, and treats blank ones as "keep current value",
// so the placeholders say so instead of suggesting defaults.
const props = defineProps<{ form: SnmpFormFields; keepBlank?: boolean }>()
