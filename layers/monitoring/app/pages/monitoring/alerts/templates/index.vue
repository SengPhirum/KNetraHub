...
async function confirmDelete(headers: Record<string, string>) {
  ...
  await $fetch(`/api/monitoring/v1/alerts/templates/${deleteTarget.value.id}`, { method: 'DELETE', headers })
...
<template>
  <div>
    ...
    <ConfirmDeleteModal
      type="monitoring.alert-template"
      :item-name="deleteTarget?.name"
      :open="!!deleteTarget"
      @update:open="(v: boolean) => { if (!v) deleteTarget = null }"
      title="Delete template"
      :message="deleteTarget ? `Template ${deleteTarget.name} will be deleted. Rules bound to it fall back to the default format.` : ''"
