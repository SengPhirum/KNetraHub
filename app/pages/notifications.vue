...
// Full notification history — the "See all" view behind the navbar bell. Same
// centralized feed (server/utils/notificationFeed.ts), with filters and paging.
// Rows are scoped server-side to the apps this user may see.
interface FeedItem {
  id: string
  app: string
...
<template>
  <div>
    <PageHeader title="Notifications" subtitle="Alerts from the portal and every app you have access to" icon="i-lucide-bell">
      <template #actions>
        <UButton
          v-if="unread"
          size="sm"
