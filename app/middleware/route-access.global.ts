...
export default defineNuxtRouteMiddleware(async (to) => {
  ...
  const { fetchModules, isEnabled } = useModules()
  ...
  if (isEnabled(app) && hasApp(app)) return

  if (import.meta.client) {
    useToast().add({
      title: isEnabled(app) ? 'No access' : 'Module disabled',
      description: isEnabled(app) ? `You don't have access to that app.` : 'An administrator must enable and initialize that module first.',
      color: 'warning'
    })
  }
  return navigateTo('/')
