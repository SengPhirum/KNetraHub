...
// Explicitly the string-valued fields only - faviconAutoGenerated/
// pwaIconAutoGenerated are server-managed bookkeeping (see
// appearanceSettings.ts) and must never be settable from the request body.
type EditableField = 'appName' | 'primaryColor' | 'logoHorizontalUrl' | 'logoIconUrl' | 'faviconUrl' | 'pwaIconUrl' | 'envBadgeColor'
const FIELDS: EditableField[] = ['appName', 'primaryColor', 'logoHorizontalUrl', 'logoIconUrl', 'faviconUrl', 'pwaIconUrl', 'envBadgeColor']
...
export default defineEventHandler(async (event) => {
  ...
  if (patch.envBadgeColor !== undefined && !HEX_COLOR.test(patch.envBadgeColor)) {
    throw createError({ statusCode: 400, statusMessage: 'Environment badge color must be a hex value like #DC2626' })
  }
