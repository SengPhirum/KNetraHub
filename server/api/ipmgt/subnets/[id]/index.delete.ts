...
import { requireIpam, ipamAudit, loadSubnet } from '../../../../utils/ipamStore'
...
export default defineEventHandler(async (event) => {
  ...
  const ipCnt = await db.query('SELECT count(*)::int AS c FROM ipmgt_ips WHERE subnet_id = $1', [id])
  const childCnt = await db.query('SELECT count(*)::int AS c FROM ipmgt_subnets WHERE parent_id = $1', [id])
  if ((Number(ipCnt.rows[0].c) > 0 || Number(childCnt.rows[0].c) > 0) && !force) {
    throw createError({
      statusCode: 409,
      statusMessage: `Subnet has ${ipCnt.rows[0].c} address(es) and ${childCnt.rows[0].c} child subnet(s). Use force delete.`
    })
  }
